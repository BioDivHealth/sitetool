...
output:
  html_fragment:
    ...
    toc_float: false  # TOC will be inline, after the title
---
...
The SiteTool is an R Shiny tool used to assess geospatial
...
NDVI) for a list of sites, and see where potential sites fall along a
gradient of other potential sites.

The tool's source code (including the `sitetool` R package) is also
available for local deployment on GitHub [link hidden for blind review], which allows larger file uploads.
...
### Step 1: Select an area of interest and type of raster data
...
2.  **Populated Places Sites**

    Populated places will find all of the towns/villages/cities in
    the selected area using OpenStreetMap. You can have the tool find all
    of the populated places in an area or randomly select a subset from
    that region. Note that if you select a very large area for populated
    place sites,
...
3.  **Data Tables and Export**
    ...
    -   **Mean Patch Area**: the mean patch size for the land cover
        category in the area analyzed. A larger mean patch area
        indicates larger and more contiguous patches of that land cover
        type.
